...
void drawObject() {
	...
	for (int j = 0; j < 7; j++)
	{
		for (int i = 0; i < 7; i++)
		{
			...
			glBindTexture(GL_TEXTURE_2D, tex0[rura[k].model]);
...
void displayFrame() {
	...
	matP=glm::perspective(cameraAngle, Width/Height, 1.0f, 100.0f);
...
/*	*/
	matV = glm::lookAt( //Wylicz macierz widoku
		...
		glm::vec3(target_x, target_y, 0.0f),
...
int main(int argc, char** argv) {
	...
	tex0[0] = readTexture("niebo.tga");
	for(int i = 1; i < LICZBAELEMENTOW; i++)
	{	       
		tex0[i] = readTexture("grass.tga");	     //textureName[i]);
	}
