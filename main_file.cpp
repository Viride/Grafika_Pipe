...
void drawObject() {
	...
	for (int j = 0; j < 7; j++)
	{
		for (int i = 0; i < 7; i++)
		{
			...
			if (rura[k].obrot == 1) {
				modele = glm::rotate(modele, 90.0f, glm::vec3(0, 1, 0)); rura[k].obrot = 0;
				printf("jestem tu\n");
			}
			...
			glUniformMatrix4fv(shaderProgram->getUniformLocation("M"), 1, false, glm::value_ptr(glm::scale(modele, glm::vec3(1))));
